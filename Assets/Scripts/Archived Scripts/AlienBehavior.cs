...
public class AlienBehavior : EnemyBehavior
{
    ...
    private void Update()
    {
        ...
        ScanPath(); //DGS - uncomment this when using Raycast
    }
    ...
    ////DGS - Comment the triggers when using Raycasts
    //private void OnTriggerEnter2D(Collider2D collision)
    //{
    //    if (!rotating && !entering)
    //    {
    //        print($"{collision.gameObject.tag} detected & rotating {rotating} & entering {entering}");
    //        moving = false;
    //        hasObstacle = true;

    //        if (collision.gameObject.CompareTag(Tags.Player) || collision.gameObject.CompareTag(Tags.Asteroid))
    //        {
    //            print("Initiating Shoot Command");
    //            Shoot();
    //        }
    //        else if (collision.gameObject.CompareTag(Tags.Alien))
    //        {
    //            print("Initiating Think Command");
    //            Thinking();
    //        }
    //    }
    ...
    //{
    //    if (collision.gameObject.CompareTag(Tags.Player) || collision.gameObject.CompareTag(Tags.Asteroid) || collision.gameObject.CompareTag(Tags.Alien))
    //    { 
    //        //when the blocker moves away
    //        print("Initiating Think Command");
    //        Thinking();
    //    }
    //}
    ...
    private void ScanPath()
    {
        if (!rotating && !entering)
        {
            ...
            if (rayHit)
            {
                ...
                if (rayHit.collider.CompareTag(Tags.Player) || rayHit.collider.CompareTag(Tags.Asteroid))
                    Shoot();
                else if (rayHit.collider.CompareTag(Tags.Alien))
                    Thinking();
            }
            else
            {
