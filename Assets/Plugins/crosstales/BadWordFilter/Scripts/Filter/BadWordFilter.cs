      ...
      public override bool isReady
      {
         get
         {
            ...
            if (!ready)
            {
               ...
               {
                  if (tempBadWordProviderLTR.Any(bp => bp != null && !bp.isReady))
                  {
                     result = false;
                  }
               }
               ...
               if (result)
               {
                  ...
                  {
                     if (tempBadWordProviderRTL.Any(bp => bp != null && !bp.isReady))
                     {
                        result = false;
                     }
                  }
               ...
               if (!readyFirstime && result)
               {
                  ...
                  {
                     foreach (var src in from bpLTR in BadWordProviderLTR where bpLTR != null from src in bpLTR.Sources where src != null where !sources.ContainsKey(src.Name) select src)
                     {
                  ...
                  {
                     foreach (var src in from bpRTL in BadWordProviderRTL where bpRTL != null from src in bpRTL.Sources where src != null where !sources.ContainsKey(src.Name) select src)
                     {
                        sources.Add(src.Name, src);
                     }
                  }
            ...
            else
            {
               ...
               if (Util.Config.DEBUG_BADWORDS)
               {
                  if (sourceNames == null || sourceNames.Length == 0)
                  {
                     if (SimpleCheck)
                     {
                        foreach (var simpleWord in from words in simpleBadwords.Values from simpleWord in words where _text.CTContains(simpleWord) select simpleWord)
                        {
                  ...
                  else
                  {
                     foreach (string badWordsResource in sourceNames)
                     {
                        if (SimpleCheck)
                        {
                           ...
                           if (simpleBadwords.TryGetValue(badWordsResource, out words))
                           {
                              foreach (var simpleWord in words.Where(simpleWord => _text.CTContains(simpleWord)))
                              {
               ...
               else
               {
                  if (sourceNames == null || sourceNames.Length == 0)
                  {
                     if (SimpleCheck)
                     {
                        foreach (var simpleWord in from words in simpleBadwords.Values from simpleWord in words where _text.CTContains(simpleWord) where !result.Contains(simpleWord) select simpleWord)
                        {
                           result.Add(simpleWord);
                        }
                     }
                  ...
                  else
                  {
                     foreach (string badWordsResource in sourceNames)
                     {
                        if (SimpleCheck)
                        {
                           ...
                           if (simpleBadwords.TryGetValue(badWordsResource, out words))
                           {
                              foreach (var simpleWord in words.Where(simpleWord => _text.CTContains(simpleWord)).Where(simpleWord => !result.Contains(simpleWord)))
                              {
                                 result.Add(simpleWord);
                              }
                           }
                        ...
                        else
                        {
                           ...
                           if (exactBadwordsRegex.TryGetValue(badWordsResource, out badWordRegex))
                           {
                              ...
                              foreach (var capture in from System.Text.RegularExpressions.Match match in matches from System.Text.RegularExpressions.Capture capture in match.Captures where !result.Contains(capture.Value) select capture)
                              {
                                 result.Add(capture.Value);
                              }
                           }
         ...
         if (isReady)
         {
            ...
            else
            {
               ...
               else
               {
                  if (sourceNames == null || sourceNames.Length == 0)
                  {
                     foreach (var capture in from badWordsResource in exactBadwordsRegex.Values select badWordsResource.Matches(_text) into matches from System.Text.RegularExpressions.Match match in matches from System.Text.RegularExpressions.Capture capture in match.Captures select capture)
                     {
                        result = replaceCapture(result, capture, markOnly, prefix, postfix, result.Length - _text.Length);
